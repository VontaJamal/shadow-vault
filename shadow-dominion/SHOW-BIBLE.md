...
**Format:** 10-15 minute episodes. Mix of structured segments and natural conversation.
...
**The Shadow Court** — A dark throne room rendered in crimson and black. Think Nazarick's throne room meets a cyberpunk war room. Multiple screens showing live data. The Monarch's throne at the center. The agents have their positions around it.

**Key locations (animated backgrounds):**

| Location | Used For | Visual |
|----------|----------|--------|
...
### Standard Episode (12-15 min)

| Timestamp | Segment | Duration | Description |
|-----------|---------|----------|-------------|
| 0:00 | **Cold Open** | 30-60s | Dramatic/funny scene that hooks. Could be mid-argument, mid-revelation, or a dramatic moment from the episode. |
| 0:30-1:00 | **Title Card + Theme** | 15-30s | "The Shadow Court" title animation. Short, punchy theme music (dark, electronic, anime OP energy). |
| 1:00 | **Court Convenes** | 1-2 min | Indra opens the session. Sets the topic. Characters react to the premise. |
| 3:00 | **Main Discussion** | 5-7 min | The meat. Characters debate, share insights, drop knowledge. Structured but allows natural tangents. |
| 10:00 | **The Verdict** | 2-3 min | Characters give final takes. Indra delivers the ruling/conclusion. |
| 12:00 | **Next Episode Tease** | 30s | Preview of next topic. Leave a hook. |
| 12:30 | **Execute. ⚔️** | 5s | Every episode ends the same way. Indra says it. Screen goes black. Swords cross. |
...
### Arc: "The Court Awakens"
The first season introduces the court, establishes character dynamics, and builds from "what is this?" to "I need the next episode."
...
## Storyline Framework: How to Write an Episode
...
```markdown
# Episode [#]: "[Title]"
## Type: [Full Court / Debate / Intel Briefing / Anime Court / Monarch Speaks]

### Cold Open
[30-60 seconds. The hook scene. Drop the audience into the most interesting moment.]

### Court Convenes
INDRA: [Sets the topic]
[Characters react]

### Main Discussion
[5-7 minutes of dialogue. Natural flow with key beats mapped out.]

BEAT 1: [Setup — establish the question]
BEAT 2: [First perspectives — 2-3 characters weigh in]
BEAT 3: [Conflict — disagreement or surprising take]
BEAT 4: [Depth — data, examples, anime parallels]
BEAT 5: [Resolution or cliffhanger]
...
- [ ] Does the cold open hook in 10 seconds?
- [ ] Is there genuine disagreement between characters?
- [ ] Does each character sound like THEMSELVES (not generic)?
- [ ] Is there at least one moment the audience will want to clip/share?
- [ ] Is there real substance (data, insight, not just vibes)?
- [ ] Does Kai make at least one anime reference?
- [ ] Does it end with Execute ⚔️?

---

## Production Pipeline: From Idea to Published

### The System (Automated Where Possible)

```
IDEA → SCRIPT → AUDIO → ANIMATE → EDIT → PUBLISH → PROMOTE
 Day 1   Day 2   Day 3    Day 4    Day 5   Day 6    Day 6+
```

### Phase-by-Phase

#### 1. IDEA (Faye handles this)
- Nightly synthesis surfaces trending topics
- Shadow Vault content ideas feed into episode topics
- Indra picks from a queue of pre-researched topics
- **File:** `episodes/ideas-queue.md`

#### 2. SCRIPT (Faye + Indra)
- Faye drafts using the template above
- Indra reviews, adjusts character positions, adds personal takes
- Script goes into `episodes/S01/E##-title/script.md`
- **Time:** 1-2 hours

#### 3. AUDIO (Automated)
- Indra records his lines (real voice)
- Faye generates agent voices via ElevenLabs API
- Each character's lines → separate audio files
- Master audio mixed in Audacity/Logic
- **File:** `episodes/S01/E##-title/audio/`
- **Time:** 1-2 hours (recording + generation + mixing)
- **Automation opportunity:** Script that takes script.md → splits by character → sends to ElevenLabs → returns audio files

#### 4. ANIMATE (Semi-automated)
- Load Live2D models into VTube Studio
- Play audio, VTube Studio lip-syncs characters
- Record each character's animation as video (OBS)
- Switch between characters based on who's speaking
- **File:** `episodes/S01/E##-title/animation/`
- **Time:** 2-3 hours
- **Automation opportunity:** Script that detects speaker changes in audio and auto-switches active character

#### 5. EDIT (Manual + Templates)
- DaVinci Resolve or Premiere
- Composite character animations over backgrounds
- Add title cards, transitions, text overlays, effects
- Add cold open, theme, end card
- **Template project file** with pre-built transitions, backgrounds, lower thirds
- **File:** `episodes/S01/E##-title/edit/`
- **Time:** 3-5 hours
- **Automation opportunity:** Episode template in DaVinci with pre-set tracks for each character

#### 6. PUBLISH
- Export final video → YouTube
- Export audio only → Spotify, Apple Podcasts (via hosting platform)
- Generate thumbnail (template + episode-specific text)
- Write show notes (Faye auto-generates from script)
- **Automation opportunity:** Script that uploads to YouTube, generates description, sets thumbnail

#### 7. PROMOTE
- Faye generates tweet thread from episode highlights
- Cut 3-4 clips (30-60s) for TikTok/Shorts
- LinkedIn post (professional angle)
- Post to Shadow Court Telegram
- **Automation opportunity:** Clip detection from script beats → auto-generate clip timestamps
...
content-vault/
└── shadow-dominion/
    ├── SHOW-BIBLE.md          (this file)
    ├── PODCAST-PRODUCTION-RESEARCH.md
    ├── episodes/
    │   ├── ideas-queue.md
    │   └── S01/
    │       ├── E01-arise/
    ...
    │       └── ...
    ├── assets/
    │   ├── characters/        (character art + Live2D files)
    │   ├── backgrounds/       (scene backgrounds)
    │   ├── music/             (theme, transitions, stingers)
    │   └── templates/         (DaVinci project, thumbnail PSD)
    └── scripts/
        ├── generate-audio.sh  (script → ElevenLabs → audio files)
        ├── publish.sh         (upload to YouTube + podcast platforms)
        └── clip-cutter.sh     (extract clips from timestamps)
```

---

## Automation Scripts to Build (Armory Candidates)

| Script | What It Does | Priority |
|--------|-------------|----------|
| **script-to-audio** | Takes script.md, splits by character, sends to ElevenLabs, returns audio files per character + master mix | High |
| **episode-publisher** | Takes final video + audio, uploads to YouTube (title, description, thumbnail, tags) and podcast host | Medium |
| **clip-cutter** | Takes timestamps from script, extracts clips from final video for Shorts/TikTok | Medium |
| **show-notes-gen** | Takes script.md, generates YouTube description, podcast show notes, tweet thread | High |
| **episode-init** | Creates the folder structure for a new episode from template | High (simple) |
